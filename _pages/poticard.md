...
- 경력·기술 스택 등록 → AI 분석 → 핵심 키워드 추출 → 디지털 명함 자동 생성
- QR 코드·고유 URL로 명함 공유, 기업 담당자가 명함 열람 후 1:1·그룹 채팅으로 채용 연락
- 탭 미접속 상태에서도 Web Push 알림으로 메시지 수신
- 포트원 API 결제 연동 — AI 분석·고급 명함 기능을 프리미엄 구독으로 제공
